...
// Returns a float3 with Fresnel term (F), cos_theta_h, and the remaining BSDF weight
                                        ...
                                        float cos_theta_i, float cos_theta_o, const float ior1_over_ior2)
{
  ...
  float cos_theta_h = dot(wh, n);
  float cos_ratio_o = wo_dot_wh/cos_theta_o;
  if(cos_ratio_o <= 0.0f || cos_theta_h < 1.0e-5f)
    return make_float3(0.0f);
  ...
  return make_float3(transmit ? 1.0f - F : F, cos_theta_h, cosine_weight);
}
                                              ...
                                              float cos_theta_o, const float ior1_over_ior2, const float s)
{
  ...
  const float cos_theta_h = bsdf.y;
  const float cosine_weight = bsdf.z;
  float cth_sqr = cos_theta_h*cos_theta_h;
  float tth_sqr = (1.0f - cth_sqr)/cth_sqr;
  float G = smith_G_beckmann(cos_theta_i, cos_theta_o, s)*cosine_weight;
  float D = M_1_PIf/(s*s*cth_sqr*cth_sqr)*expf(-tth_sqr/(s*s));
  return F*D*G; // bsdf * cos_theta_i
}
                                         ...
                                         float cos_theta_o, const float ior1_over_ior2, const float s)
{
  ...
  float D = M_1_PIf*s*s/sqr(cth_sqr*(s*s + tth_sqr));
  return F*D*G; // bsdf * cos_theta_i
}
                                ...
                                float cos_theta_o, const float ior1_over_ior2, const float xi)
{
  ...
  float cosine_weight = cos_ratio_o/cos_theta_h;
...
// Returns Fresnel term in F and a float2 with cos_theta_h and the remaining BSDF weight
                                        ...
                                        float cos_theta_i, float cos_theta_o, const complex3& recip_ior, float3& F)
{
  ...
  float cosine_weight = transmit
    ? fabsf(wi_dot_wh)*cos_ratio_o/sqr(ior1_over_ior2*wi_dot_wh + wo_dot_wh)
    : 0.25f/fabsf(cos_theta_o);
  return make_float2(cos_theta_h, cosine_weight);
}
                                            ...
                                            float cos_theta_o, const complex3& recip_ior, const float s)
{
  ...
  const float cos_theta_h = bsdf.x;
