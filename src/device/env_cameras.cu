...
extern "C" __global__ void __raygen__env_luminance()
{
  ...
  lp.env_luminance[pixel_idx] = luminance_NTSC(texel)*sinf(theta);
}
...
extern "C" __global__ void __raygen__env_pdf()
{
  ...
  const uint3 launch_dims = optixGetLaunchDimensions();
  const unsigned int pixel_idx = launch_idx.y*launch_dims.x + launch_idx.x;
  lp.conditional_pdf[pixel_idx] = lp.env_luminance[pixel_idx]/lp.marginal_f[launch_idx.y];
  float cdf_sum = 0.0f;
  ...
  lp.conditional_cdf[pixel_idx] = cdf_sum/lp.marginal_f[launch_idx.y];
  if(launch_idx == launch_dims - make_uint3(1u))
    lp.conditional_cdf[pixel_idx] = 1.0f;  // handle numerical instability
...
__device__ __inline__ void sample_environment(const float3& pos, float3& dir, float3& L, unsigned int& t)
{
  ...
  float3 emission = make_float3(tex2D<float4>(lp.envmap, u, v));
