...
struct Photon {
  float3 throughput;  // Energy * Geometric Term / pdf
  float3 position;
  float3 wi;  // incident direction

  // // implementation of Point concept
  // static constexpr int dim = 3;
  // SUTILFN float operator[](int i) const { return position[i]; }

  Photon() {}
  Photon(const float3& throughput, const float3& position, const float3& wi)
...
inline float distance2(const float3& p1, const float3& p2) {
  float dist2 = 0;
  for (int i = 0; i < 3; ++i) {
