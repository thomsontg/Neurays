...
namespace
{
  ...
  {
    return accumulate(begin(v), end(v), static_cast<T>(0))/static_cast<double>(v.size());
  }
  ...
  template<class T> Color<T> spectrum_to_rgb(const Color<T>& spec)
  {
    ...
    for(unsigned int i = 0; i < spec.size(); ++i)
    {
      ...
      double lambda_clamped = std::min(std::max(lambda, spectrum_rgb[0][0]), spectrum_rgb[spec.size() - 1][0]);
      double c = (lambda_clamped - spectrum_rgb[idx][0])/(spectrum_rgb[idx + 1][0] - spectrum_rgb[idx][0]);
      for(unsigned int j = 1; j < 4; ++j)
      {
  ...
  template<class T> Color<T> xyz_to_spectrum(const Color<T>& v_xyz, size_t samples)
  { 
    return Color<T>();
  }
  ...
  template<class T> void multi_to_spectrum(Color<T> param[4], size_t samples)
  {
    if(param[xyz].size() == 3)
...
void Medium::fill_rgb_data(bool verbose)
{
  if(verbose)
    cout << "Optical properties of " << name << endl;
  if(ior[rgb].size() != 3)
  {
    multi_to_rgb(ior);
    if(ior[rgb].size() != 3)
  ...
  if(verbose)
    cout << "Refractive index: [ " << ior[rgb][0] << ", " << ior[rgb][1] << ", " << ior[rgb][2] << " ]" << endl;
  ...
  if(verbose)
    cout << "Absorption coefficient [1/m]: [ " << absorption[rgb][0] << ", " << absorption[rgb][1] << ", " << absorption[rgb][2] << " ]" << endl;
  ...
  if(emissive && emission[rgb].size() != 3)
  {
    ...
    for(unsigned int i = 0; i < 3; ++i)
      if(emission[rgb][i] < 0.0)
        emission[rgb][i] = 0.0;
    if(verbose)
      cout << "Emission [radiance]: [ " << emission[rgb][0] << ", " << emission[rgb][1] << ", " << emission[rgb][2] << " ]" << endl;
  }
  ...
  if(turbid)
  {
    ...
    if(verbose)
    {
      cout << "Scattering coefficient [1/m]: [ " << scattering[rgb][0] << ", " << scattering[rgb][1] << ", " << scattering[rgb][2] << " ]" << endl;
      cout << "Asymmetry parameter: [ " << asymmetry[rgb][0] << ", " << asymmetry[rgb][1] << ", " << asymmetry[rgb][2] << " ]" << endl;
    }
  }
  else
  {
...
void Medium::fill_spectral_data()
{
  ...
  size_t samples = ior[spectrum].size();
